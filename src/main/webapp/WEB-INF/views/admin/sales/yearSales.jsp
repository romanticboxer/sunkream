...
<!-- 제목 -->
<font size="10px"><b>${year}년 매입 매출</b></font>
...
for(i=1;i<=12;++i){
	...
	for(k=0;k<list.length;++k){
		if(list[k].sales_type == 'out'){
			...
			var month = reg_date.split("/")[1];
			if(month == j) outcome[i-1] += list[k].money;
		}
		if(list[k].sales_type == 'in'){
			var reg_date = list[k].reg_date;
			var month = reg_date.split("/")[1];
			if(month == j) income[i-1] += list[k].money;
		}
